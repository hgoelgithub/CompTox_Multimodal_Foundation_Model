...
def build_model(checkpoint,tok,device='cpu'):
    """Construct an untrained model with the checkpoint's architecture."""
    cfg=checkpoint['config']; m=cfg['model']
    return CompToxFoundationModel(len(tok.vocab),checkpoint['modality_dims'],
...
def load_checkpoint(path, device='cpu'):
    ...
    tok=SmilesTokenizer(checkpoint['vocab'])
    model=build_model(checkpoint,tok,device)
